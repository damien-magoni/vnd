...
*/				kill(); // works if called only once!
	...
	*/
	...
	void end_point::handle_read(const boost::system::error_code & ec, std::size_t bytes_transferred)
	{
		...
		{
			vnd_pt->io_serv->post(boost::bind(&input_hold_queue::preprocess, bound_if->input_queue, f)); // concur access needed?
		}
		else
		{
			vnd_pt->destroy_frame(f);
			vnd_pt->warning << "warning> ep not bound to input line interface, dropping frame\n";
		}
		reread_loop();
	}
