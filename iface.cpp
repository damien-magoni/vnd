...
namespace vnd
{
	...
	hold_queue::~hold_queue()
	{
		if ( line_pt->vnd_pt->debug ) 
			line_pt->vnd_pt->warning << "hold_queue base dtor called\n";
		if ( timer )
		{
...
///////// begin of input_hold_queue

	input_hold_queue::input_hold_queue(boost::asio::io_service * io_serv) : hold_queue(io_serv)
	{
		input = true;
	}

	input_hold_queue::~input_hold_queue()
	{
	}

	std::string input_hold_queue::print_id()
	{
		return line_pt->id + "-in";
	}

	void input_hold_queue::wait(frame * f)
	{
...
*/
		if ( expiration_time > boost::chrono::high_resolution_clock::now() ) // wait
		{
			...
			{
				line_pt->vnd_pt->error << "[input_queue->wait] expires_at exception: " <<  e.what() << "\n";
			}
			if ( asyn_ops_cancelled ) // there were pending ops, impossible! we managed to cancel the timer, start new asynchronous wait
			{
				line_pt->vnd_pt->warning << "[input_queue->wait] expires_at has cancelled " << static_cast<dword>(asyn_ops_cancelled) << " operations\n";
			}
			timer_running = true;
			timer->async_wait(boost::bind(&input_hold_queue::handle_wait, this, boost::asio::placeholders::error, f));
		}
	...
	void input_hold_queue::handle_wait(const boost::system::error_code & ec, frame * f)
	{
		timer_running = false;
		...
		if ( line_pt->trace_flag || line_pt->trace_file_flag )
			line_pt->trace(f, 0);
		line_pt->vnd_pt->io_serv->post(boost::bind(&network_device::forward, line_pt->vnd_pt, f));
		schedule();
	}

	// called by handle_read/input or inj/input
	void input_hold_queue::preprocess(frame * f)
	{
...
*/		if ( false )
	...
	// called inside output/schedule before write_loop: wrong because of copies!!!
	// called inside forward_mobile before forward_hub
	void input_hold_queue::postprocess(frame * f)
	{
	...
	void input_hold_queue::push(frame * f)
	{
		if ( !up )
		{
			//if ( line_pt->vnd_pt->debug ) 
	...
	void input_hold_queue::schedule()
	{
...
// output_hold_queue
	...
	output_hold_queue::output_hold_queue(boost::asio::io_service * io_serv) : hold_queue(io_serv)
	{
		input = false;
	}

	output_hold_queue::~output_hold_queue()
	{
	}

	std::string output_hold_queue::print_id()
	{
		return line_pt->id + "-out";
	}

	void output_hold_queue::wait(frame * f)
	{
		dword bits = f->buffer.size() * 8;
		double fp_ns_per_bit = static_cast<double>(1.0E9) / static_cast<double>(bw);
		double fp_ns_per_frame = static_cast<double>(bits) * fp_ns_per_bit;
...
*/		{
		...
		if ( expiration_time > boost::chrono::high_resolution_clock::now() ) // wait
		{
			...
			timer_running = true;
			timer->async_wait(boost::bind(&output_hold_queue::handle_wait, this, boost::asio::placeholders::error, f));
		}
		else // act immediately
		{
			boost::system::error_code ec;
	...
	void output_hold_queue::handle_wait(const boost::system::error_code & ec, frame * f)
	{
		timer_running = false;
		if ( ec ) // used by async_wait
		{
	...
	void output_hold_queue::push(frame * f)
	{
		if ( !up )
		{
			//if ( line_pt->vnd_pt->debug ) 
	...
	void output_hold_queue::schedule()
	{
		if ( timer_running ) // here to avoid calling async_wait on running timer while having already popped up a frame
		{
			return;
		...
		if ( line_pt->bound_ep && line_pt->bound_ep->write_overlap )
		{
	...
	line_interface::line_interface(boost::asio::io_service * io_serv)
	{
		...
		bound_ep = 0;
		input_queue = new input_hold_queue(io_serv);
		input_queue->input = true;
		input_queue->line_pt = this;
		output_queue = new output_hold_queue(io_serv);
